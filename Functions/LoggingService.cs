...
namespace Astro_New_Joiner.Functions
{
    public class LoggingService
    {
        ...
        public  Task CommandExecutedAsync(Optional<CommandInfo> command, ICommandContext context, IResult result)
        {
            ...
            { 
                logText = $"{DateTime.UtcNow:hh:mm:ss} [Msg] Successful command '{command.Value.Name}' executed by user: '{context.User.Username}'.";
            }
            else
            {
                logText = $"{DateTime.UtcNow:hh:mm:ss} [Msg] Invalid command '{context.Message}' attempted by user:'{context.User.Username}' {result.ErrorReason}";
            }
